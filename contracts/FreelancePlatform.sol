...
contract FreelancePlatform is Ownable, ReentrancyGuard, AccessControl {
    ...
    uint256 constant SUBMISSION_END_BUFFER = 10 days;
    uint256 constant FINAL_SUBMISSION_END_BUFFER = 10 days;
    uint256 constant EXTENSION_REQUEST_CUTOFF_BUFFER = 2 days;
    uint256 constant REVIEW_PERIOD = 10 days;
    uint256 constant AUTO_APPROVE_PERIOD = 10 days;
    ...
    event MilestoneFinalSubmitted(uint256 indexed milestoneId, uint256 indexed projectId, uint256 finalSubmitTime, address freelancer);
    ...
    function createProject(
        ...
        projects[projectCounter] = Project({
            ...
            applicationDeadline: block.timestamp + (_applicationPeriodDays * 1 days)
        });
        ...
        onlyRegisteredUser(msg.sender)
    {
        ...
        require(block.timestamp <= project.applicationDeadline, "Application deadline passed");
    ...
    function shortlistFreelancers(
        ...
        for (uint i = 0; i < _freelancers.length; i++) {
            require(hasApplied[_projectId][_freelancers[i]], "Freelancer hasn't applied");
    ...
    function selectFreelancer(uint256 _projectId, address _freelancer) 
        external 
        projectExists(_projectId) 
        onlyClient(_projectId)  
    {
    ...
    function agreeMilestones(
        ...
        require(_amounts.length == _deadlines.length && _amounts.length == _metadataHashes.length, "Array mismatch");
    ...
    function submitMilestoneWork(
    ...
    ) external onlyFreelancer(milestones[_milestoneId].projectId) milestoneExists(_milestoneId) {
        Milestone storage milestone = milestones[_milestoneId];
        require(milestone.status == MilestoneStatus.Pending, "Invalid milestone status");
        require(block.timestamp <= milestone.deadline + 3 days, "Submission period expired");
        ...
        emit MilestoneSubmitted(_milestoneId, milestone.projectId, milestone.amount);
    }
    
    function finalSubmitMilestone(uint256 _milestoneId)
        external
        ...
        milestoneExists(_milestoneId)
    {
        ...
        milestone.status = MilestoneStatus.Submitted;
        emit MilestoneFinalSubmitted(_milestoneId, milestone.projectId, block.timestamp, msg.sender);
    }
        ...
        milestoneExists(_milestoneId)
    {
        ...
        Milestone storage milestone = milestones[_milestoneId];
        require(milestone.status == MilestoneStatus.Submitted, "Milestone not submitted or already processed");
        require(!milestone.disputeRaised, "Milestone is under dispute");
        
        uint256 submitTime = milestone.submissionTime > 0 ? milestone.submissionTime : milestone.finalSubmitTime;
        require(block.timestamp <= submitTime + REVIEW_PERIOD, "Approval period expired");
    ...
    function autoApproveMilestone(uint256 _milestoneId) 
        external 
        projectExists(milestones[_milestoneId].projectId) 
        nonReentrant
        milestoneExists(_milestoneId)
    {
        ...
        uint256 submitTime = milestone.submissionTime > 0 ? milestone.submissionTime : milestone.finalSubmitTime;
        require(block.timestamp > submitTime + AUTO_APPROVE_PERIOD, "Auto-approve period not reached");
        require(project.escrowBalance >= milestone.amount, "Insufficient balance");
        ...
        emit MilestoneApproved(_milestoneId, projectId, address(0));
        emit PaymentReleased(_milestoneId, projectId, netAmount, project.freelancer);
        ...
        milestoneExists(_milestoneId)
    {
        ...
        Milestone storage milestone = milestones[_milestoneId];
        require(block.timestamp <= milestone.deadline + SUBMISSION_END_BUFFER - EXTENSION_REQUEST_CUTOFF_BUFFER, "Cannot request extension within last 48 hours of grace period");
        require(_newDeadline > milestone.deadline, "New deadline must be later than current deadline");
        uint256 maxAllowedDeadline = milestone.deadline + SUBMISSION_END_BUFFER;
        require(_newDeadline <= maxAllowedDeadline, "Extension exceeds maximum allowed period");
    ...
    function approveExtension(uint256 _milestoneId, uint256 _newDeadline) 
        external 
        projectExists(milestones[_milestoneId].projectId) 
        onlyClient(milestones[_milestoneId].projectId)
        milestoneExists(_milestoneId)
    {
        ...
        Milestone storage milestone = milestones[_milestoneId];
        require(milestone.finalSubmitTime == 0, "Cannot extend after final submission");
        require(_newDeadline > milestone.deadline, "New deadline must be later than current deadline");
        uint256 maxAllowedDeadline = milestone.deadline + SUBMISSION_END_BUFFER;
        require(_newDeadline <= maxAllowedDeadline, "Extension exceeds maximum allowed period");
    ...
    function disputeMilestone(uint256 _milestoneId)
        external
        projectExists(milestones[_milestoneId].projectId)
        ...
        milestoneExists(_milestoneId)
    {
        ...
        uint256 submitTime = milestone.submissionTime > 0 ? milestone.submissionTime : milestone.finalSubmitTime;
        require(submitTime != 0, "Milestone not finalized");
        require(block.timestamp <= submitTime + DISPUTE_WINDOW, "Dispute window expired");
    ...
    function resolveDispute(uint256 _milestoneId, address _winner, uint256 _disputedAmount) 
        external 
        onlyOwner
        milestoneExists(_milestoneId)
    {
        ...
        if (_winner == projects[projectId].freelancer) {
            if (milestones[_milestoneId].status != MilestoneStatus.Paid) {
                ...
                _updateFreelancerReputation(projects[projectId].freelancer, _disputedAmount);
        ...
        if (projects[projectId].completedMilestones == projects[projectId].totalMilestones) {
            _finalizeProject(projectId);
        }
    ...
    function _updateFreelancerReputation(address _freelancer, uint256 _amount) internal {
        ReputationData storage rep = reputations[_freelancer];
        rep.totalEarned += _amount;
        rep.projectsCompleted++;
        
        // Calculate new reputation score based on earnings and completed projects
        ...
        onlyProjectParticipant(_projectId) 
    {
        ...
        if (msg.sender == projects[_projectId].client) {
            ...
            rep.totalRatings++;
            rep.averageRating = ((oldAverage * oldCount) + _rating) / rep.totalRatings;
        }
    ...
    function withdrawExcessFunds(uint256 _projectId) 
        external 
        onlyClient(_projectId) 
        nonReentrant 
    {
        ...
        uint256 excessAmount = project.escrowBalance - pendingAmounts[_projectId];
        if (excessAmount > 0) {
    ...
    function getShortlistedFreelancers(uint256 _projectId) 
        external 
        view 
        projectExists(_projectId) 
    ...
    function hasFreelancerApplied(uint256 _projectId, address _freelancer) 
        external 
        view 
        projectExists(_projectId) 
        returns (bool) 
    ...
    function getPendingAmount(uint256 _projectId) 
        external 
        view 
        projectExists(_projectId) 
        returns (uint256) 
    ...
    {
        uint256[] memory projectMilestones = new uint256[](milestoneCounter);
        uint256 count = 0;
        
        for (uint256 i = 1; i <= milestoneCounter; i++) {
            if (milestones[i].projectId == _projectId) {
                projectMilestones[count] = i;
                count++;
            }
        }
    ...
    function getFreelancerProjects(address _freelancer) 
        ...
        returns (uint256[] memory) 
    {
        uint256[] memory freelancerProjects = new uint256[](projectCounter);
        uint256 count = 0;
        
        for (uint256 i = 1; i <= projectCounter; i++) {
            if (projects[i].freelancer == _freelancer) {
                freelancerProjects[count] = i;
                count++;
            }
        }
    ...
    function isUsingEnhancedFlow(uint256 _projectId) 
        external 
        view 
        projectExists(_projectId) 
        returns (bool) 
    ...
    function migrateProjectToEnhanced(
        uint256 _projectId,
        string calldata _metadataHash,
        uint256 _applicationDays
    ) external onlyClient(_projectId) projectExists(_projectId) {
        Project storage project = projects[_projectId];
